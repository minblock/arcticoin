...
const char * const BITCOIN_CONF_FILENAME = "arcticoin.conf";
...
#else
    const char* pszModule = "arcticoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "arcticoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/arcticoin";
#else
    // Unix
    return pathRet / ".arcticoin";
#endif
#endif
}
