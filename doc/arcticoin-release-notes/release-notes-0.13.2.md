arcticoin Core version 0.13.2 is now available from:

  <https://download.arcticoin.org/arcticoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/arcticoin-project/arcticoin/issues>
...
updates will be released anymore. Without security updates, using a arcticoin
...
ECDSA signatures inside arcticoin transactions now use validation using
...
Previous versions of arcticoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of arcticoin Core was to raise the default minimum
relay fee.

arcticoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
arcticoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running arcticoind without having to do any manual configuration.
...
arcticoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. arcticoin Core will relay transactions with
...
entirely in arcticoin Core 0.13, and it is at this time undecided whether the
...
arcticoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
arcticoin Core automatically creates a hidden service to listen on, without
manual configuration. arcticoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if arcticoin Core is listening, and
...
arcticoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals arcticoin
Core to use floating fees. By default, arcticoin Core will use floating
fees.
...
At all times, arcticoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ATC.
Furthermore, arcticoin Core will never create transactions paying less than
...
`arcticoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /arcticoinrpc>
        ...
        # optional bypass arcticoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< arcticoinrpc:<password>
    </Location>
...
Restarting arcticoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in arcticoin.conf
...
arcticoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/arcticoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The arcticoin Core code base has
...
- `arcticoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `arcticoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the arcticoin scripting system that in combination with
...
arcticoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of arcticoin Core.
...
The two phases are distinct in the arcticoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming arcticoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for arcticoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each arcticoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of arcticoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of arcticoins being spent is exactly
...
- **Increased security for multisig:** arcticoin addresses (both P2PKH addresses
  ...
  believed to be provided by arcticoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of arcticoin's current ledger (called the UTXO set),
  ...
  note that arcticoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  arcticoin users to individually opt-in to almost any change in the arcticoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and arcticoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the arcticoin
client to arcticoin Core 0.13.2 to signal for segwit support. When a
...
Since arcticoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `arcticoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `arcticoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
